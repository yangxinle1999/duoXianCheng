 ...
 * CountDownLatch不涉及锁定，当count的值为零是当前线程继续运行（多个线程可以考虑多个门闩）
...
public class my5 {
    ...
    public static void main(String[] args) {
        ...
        CountDownLatch latch=new CountDownLatch(1);  //声明一个门闩

        new Thread(()->{
            System.out.println("t2启动");
            if (c.size()!=5){
                try {
                    latch.await();  //门闩等待，不需要锁定任何对象
        ...
        new Thread(()->{
            ...
            for (int i = 0; i < 10; i++) {
                ...
                if (c.size()==5){
                    //打开门闩，让t2得以执行，countDown使CountDownLatch减1，当CountDownLatch
                    //为0时，唤醒等待的门闩，执行下面的代码
                    latch.countDown();
                }
